...
#define   Peltierp_on        output_bit(PIN_C2,1)
#define   Peltierp_off       output_bit(PIN_C2,0)
#define   Peltiern_on        output_bit(PIN_C1,1)
#define   Peltiern_off       output_bit(PIN_C1,0)
#define   Buzzer_on          output_bit(PIN_C6,1)
#define   Buzzer_off         output_bit(PIN_C6,0)
#define   Vent_on            output_bit(PIN_C7,1)
#define   Vent_off           output_bit(PIN_C7,0)
...
float X[2]={0.0,0.0};
int8 h=0,l=0,Ajuste=0,Ajusten=0,MediaMovil=10,tiemporecta=0,flag_ganancia=0,saturado=0,Sentido=0,Controlando=0;
short Flanco=0,Flanco1=0,Flanco2=0,visual=1,Lectura=0,LecturaTemperatura=0,flag_ajuste=0,flag_ajuste2=0;
...
int8 y=0,tiemporeset=0,Enfria=0;
int8 Menu=0, unidad=1,i=0,Pausa=0,Pausa_ant=0,TiempoControl=0;
int8 Opcion=1,tiempo_esterilizacion=0,tiempo_secado=0;
int16 tiempos=0,tiempos3=0,tiempoctrl=0,tinicio=0,tiempocalentamiento=0,tiemposetpoint=0,Setpoint=0,Tp=0,Tc=0;
...
float V0=0.0,R1=6210.0,Pt,Tciclo=10.0,Tpulso=0.0,error=0.0,Prom=0.0,Temperature=0.0,tmp=0.0,Ganancia=25.0,Ganancia2=20.0,desvio=1.0,Coeficiente=150.0;
...
float Leer_Sensor(int media)
{
   ...
   V0 = V0/41.62; // Se elimina la ganancia
...
//-------------------------------Control de Calentamiento------------------------------------------------------------//         
   ...
   else
   {
      ...
      if(error>1.6 && error<30 && Aumento<0.3)
      {
         flag_ajuste=0;
         flag_ajuste2=0;
         if(Potencia<90.0)
...
void temp1s(void){
   ...
   {tiempos=0;Lectura=1;tiemporeset++;TiempoControl++;tiemporecta++;LecturaTemperatura=1;
...
{
   delay_ms(1000);   
   Resistencia_off;Peltierp_off;Peltiern_off;Buzzer_off;
   ...
   delay_ms(50);   
   Setpoint=make16(read_eeprom(81),read_eeprom(80));
   delay_ms(50);   
...
//_--------------------------------------------------------------------------------------------------------------------------------------------------------      
      ...
      strcpy(Menus.Letra,"C");displayMenu(Menus.Letra,92,30,2,3);
         ...
         {
            delay_ms(700);Menu=1;glcd_fillscreen(OFF);Opcion=1;visual=1;
            write_eeprom(80,make8(Setpoint,0));
            delay_ms(50);   
            write_eeprom(81,make8(Setpoint,1));//Guardar valor de Setpoint en eeprom
            delay_ms(50);   
         }
     }
//_--------------------------------------------------------------------------------------------------------------------------------------------------------    
