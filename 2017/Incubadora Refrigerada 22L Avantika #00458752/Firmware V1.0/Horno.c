...
#define   Buzzer_on          output_bit(PIN_C1,1)
#define   Buzzer_off         output_bit(PIN_C1,0)
...
float X[2]={0.0,0.0};
int8 h=0,l=0,Ajuste=0,Ajusten=0,MediaMovil=10,tiemporecta=0,flag_ganancia=0,saturado=0;
...
//-------------------------------Control de Calentamiento------------------------------------------------------------//         
   ...
   {
      Peltier_off;
      if(error>1.2 && error<50 && Aumento<0.3 && flag_setpoint==0)
      {
         if(Potencia<90.0)
         {
            if(TiempoControl>=10)
