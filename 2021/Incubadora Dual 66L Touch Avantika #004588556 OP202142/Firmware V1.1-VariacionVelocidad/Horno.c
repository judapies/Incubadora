...
int8 Opcion=1,tiempo_esterilizacion=0,tiempo_secado=0,decimal=0,entero=0,tServo=0;
int16 tiempos=0,tiempos3=0,tiempoctrl=0,tinicio=0,tiempocalentamiento=0,tiemposetpoint=0,Tp=0,Tc=0,TiempoControl=0,tiempoApertura=0,tSobrepaso=0,tiempoServo=0,TMR0_16=0;
...
void temp2s(void){
   tServo++;
   if(tServo>=2){
      output_toggle(PIN_A2);    //Encender PWM
      delay_us(ang);         //Esperar 1ms
      output_low(PIN_A2);     //Apagar PWM
      set_timer2(TMR0_16);      //Precargar el timer0
      tServo=0;
   }
}
...
while(true)
{
   ...
   Envio_Esclavos();
      ...
      ang=(angulo*11.1112)+1000;//Calcula el delay para el ancho del pulso en us
      T=20000-ang;//Calcula el restante del ancho de pulso para configurar posteriormente el timer
      T=T/1000000;//Convierte el tiempo en unidades de segundos
      TMR0=((T/0.0000512)-1)/2;// Calcula el Timer 2 con prescaler de 16 y postcaler de 16 para 20MHz
      TMR0_16=(int16)TMR0;
...
//_--------------------------------------------------------------------------------------------------------------------------------------------------------
 ...
 //----------------Muestra Temperatura--------------------------
         ...
         displayTiempo(angulo,97,13,3,1);
         //displayTiempo(angulo,97,13,3,1);
         strcpy(Menus.Letra,"°");displayMenu(Menus.Letra,115,13,2,1);//glcd_circle(120,15,1,NO,ON);
