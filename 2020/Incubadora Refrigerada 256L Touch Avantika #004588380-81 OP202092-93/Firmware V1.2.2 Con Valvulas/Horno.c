...
#define   Ventilador_on     output_bit(PIN_D2,0)// Ventilador Interno de la camara
#define   Ventilador_off    output_bit(PIN_D2,1)// Ventilador Interno de la camara
#define   Compresor_on      output_bit(PIN_D3,0)// Compresor de sistema de refrigeracion 
#define   Compresor_off     output_bit(PIN_D3,1)// Compresor de sistema de refrigeracion 
#define   Valvula1_on       output_bit(PIN_D1,0)// Valvula para conmutar de un sistema de refrigeracion a otro--Sistema Principal
#define   Valvula1_off      output_bit(PIN_D1,1)// Valvula para conmutar de un sistema de refrigeracion a otro--Sistema Principal
#define   Valvula2_on       output_bit(PIN_C5,0)// Valvula para conmutar de un sistema de refrigeracion a otro--Sistema Secundario
#define   Valvula2_off      output_bit(PIN_C5,1)// Valvula para conmutar de un sistema de refrigeracion a otro--Sistema Secundario
...
int16 tiempos=0,tiempos3=0,tiempoctrl=0,tinicio=0,tiempocalentamiento=0,tiemposetpoint=0,Tp=0,Tc=0,tDefrost=0,tCongelamiento=0,tApagadoCompresor=240,derivativo_ant=0,tterm=0,tdoor=0;
...
void temp1s(void){
   ...
   if(tiempos>=200){  // 12000 para que incremente cada minuto, 200para que incremente cada segundo.
      ...
      if(input_state(PIN_D3)){ //Salida Compresor- Compresor Apagado
         if(tApagadoCompresor>0){
            tApagadoCompresor--;
         }
      }else{
         if(input_state(PIN_D1) && input_state(PIN_C5)){//Salidas de valvulas- apagadas
            Valvula1_on;
            Valvula2_off;
         }
      }
...
void main()
{
   ...
   while(true)
   {
      if(TemperaturaEvaporador<=-1.0){ // Si el evaporador esta congelando
         flagTDefrost=ON; // Activa conteo de tiempo de congelamiento
      }else{
         flagTDefrost=OFF; // Desactiva conteo de tiempo de congelamiento
      }
      Envio_Esclavos();
         ...
         if(!rampa){
            ...
            if(RX_Buffer[4]==0x11 || LEFT){//Selecciono Regresar
               ...
               if(RX_Buffer[4]==0x11 || LEFT){
                  ...
                  SendDataDisplay();  
                  if(!GuardaEEPROM){
                     write_eeprom(0,(int8)mEstp);
                     delay_ms(10);
                     write_eeprom(1,(int8)hEstp);
                     delay_ms(10);
                     //mEst=mEstp;
                     //hEst=hEstp;
                     if((mEstp==0)&&(hEstp==0))
...
//_--------------------------------------------------------------------------------------------------------------------------------------------------------      
    if(Menu==6){ // Menu de Prueba de Componentes
         PruebaEstados=ON;
         Controlando=0;
         if(RX_Buffer[4]==0x01){//Oprimio Compresor
            EstadoR1=!EstadoR1;
            RX_Buffer[4]=0x00;
            RX_Buffer2[4]=0x00;
         }
         ...
         if(RX_Buffer[4]==0x02){//Oprimio Ventilador Camara
            EstadoR2=!EstadoR2;
            RX_Buffer[4]=0x00;
            RX_Buffer2[4]=0x00;
         }
         ...
         if(RX_Buffer[4]==0x03){//Oprimio Resistencia Evaporador
            EstadoR3=!EstadoR3;
            RX_Buffer[4]=0x00;
            RX_Buffer2[4]=0x00;
         }
         ...
         if(RX_Buffer[4]==0x04){//Oprimio Resistencia Calefaccion
            EstadoR4=!EstadoR4;
            RX_Buffer[4]=0x00;
            RX_Buffer2[4]=0x00;
         }
         ...
         if(RX_Buffer[4]==0x05){//Oprimio Valvulas
            //EstadoR5=!EstadoR5;
         ...
         if(RX_Buffer[4]==0x06){//Oprimio EV Suministro
            //EstadoR6=!EstadoR6;
            output_bit(PIN_C5,!input_state(PIN_C5));
            RX_Buffer[4]=0x00;
            RX_Buffer2[4]=0x00;
         }
         ...
         */
         ...
         SendDataDisplay();
         printf("b4.txt=\"Valvula 1\"");
         SendDataDisplay();
         printf("b5.txt=\"Valvula 2\"");
         SendDataDisplay();
...
//_--------------------------------------------------------------------------------------------------------------------------------------------------------
 ...
 //----------------Muestra Temperatura--------------------------
         if(Sensor[0].Pt>130 || Sensor[0].Pt<80){
         ...
         if(Enfria==0){
            ...
            printf("potT.txt=\"Pot. Enfria:%2.0f %%\"",100.0*(float)!input_state(PIN_D3));
            //printf("potT.txt=\"Pot:%2.0f%% %2.1f %2.4f\"",Potencia,TemperaturaIdeal,Pendiente);
            SendDataDisplay(); 
         }
         printf("texto1.txt=\"Tiempo Congelamiento:%03Lu min\"",tCongelamiento/60);
         SendDataDisplay();
         printf("texto2.txt=\"Tiempo Arranque:%03Lu s\"",tApagadoCompresor);
         SendDataDisplay();
         printf("texto3.txt=\"Tiempo Defrost:%03Lu s\"",tDefrost);
         SendDataDisplay();
         printf("texto4.txt=\"T. Evaporador:%2.1f°C\"",TemperaturaEvaporador);
         SendDataDisplay();
      } 
      ...
      if(RX_Buffer[4]==0x11 || (RIGHT && LEFT)){//Tecla Regresar
         ...
         if(RX_Buffer[4]==0x11 || (RIGHT && LEFT)){
            if(!GuardaEEPROM){
               ...
               delay_ms(10);
               write_eeprom(30,(int8)GananciaR);
               delay_ms(10);
               write_eeprom(34,(int8)(Histeresis*10.0));
               delay_ms(10);
