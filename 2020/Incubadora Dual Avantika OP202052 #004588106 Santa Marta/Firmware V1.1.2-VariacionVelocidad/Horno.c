...
int8 h=0,l=0,Ajuste=0,Ajusten=0,MediaMovil=10,tiemporecta=0,flag_ganancia=0,Lectura2=0,Controlando=0,Ajusto=10,n=0,Programa=0;
short Flanco=0,Flanco1=0,Flanco2=0,visual=1,Lectura=0,flag_ajuste=0,flag_ajuste2=0,flag_clear=0,Infinito=0,abrio=0,flagGuardaEstado=OFF;
short flag_clear2=0,flag_clear3=0,flag_tiempocalentamiento=0,flag_display=0,flag_setpoint=0,Finalizo=0,sobrepaso=0,flagControlT=OFF;
int8 y=0,tiemporeset=0,Source=1,VelMotor=0,Vel1=10,controlT=0;
...
int16 tiempos=0,tiempos3=0,tiempoctrl=0,tinicio=0,tiempocalentamiento=0,tiemposetpoint=0,Tp=0,Tc=0,TiempoControl=0,tiempoApertura=0,tSobrepaso=0,tiempoServo=0,tleft=0;
...
//_--------------------------------------------------------------------------------------------------------------------------------------------------------
      if(Menu==20){ //Menu de Funcionamiento
         if(!flagGuardaEstado){
            write_eeprom(29,1);delay_ms(20);
            write_eeprom(30,Programa);delay_ms(20);
            flagGuardaEstado=ON;
         }
      ...
      */
      ...
      if(DOOR){
         ...
         flag_tiempocalentamiento=0;
         if(flag_clear2==0){
            Ganancia=gananciaAnt*(Setpoint/150);
         }
         glcd_rect(0, 55, 127, 63, YES, OFF);
         tiempo_esterilizacion=0;
         SSR_off;
 ...
 //----------------Muestra Temperatura--------------------------
      ...
      if(Finalizo){
         ...
         tiempo_esterilizacion=0;
         if(flagGuardaEstado){
            write_eeprom(29,0);delay_ms(20);
            flagGuardaEstado=OFF;
         }
      }
      
      CuentaTiempo();
      if(tleft>400){//Si oprime boton de Toma.
         Menu=1;glcd_fillscreen(OFF);Opcion=1;visual=1;Pausa=1;
         write_eeprom(29,0);delay_ms(20);               
         Controlando=0;
         SSR_off;
         SSR2_off;
